...
function DefaultNavbar({ brand, routes, transparent, light, action, sticky, relative, center }) {
  ...
  const renderRoutes = routes.map(({ name, collapse, columns, rowsPerColumn }) => {
    ...
    if (collapse && columns && name === dropdownName) {
                  ...
                  <Fragment key={col.name}>
                    ...
                    {col.collapse
                      .filter(
                        (item) =>
                          !(
                            (authenticated && item.notAuthRequired) ||
                            (!authenticated && item.authRequired)
                          )
                      )
                      .map((item) => (
                        <MKTypography
      ...
      template = collapse
        .filter(
          (item) => (!authenticated && item.notAuthRequired) || (authenticated && item.authRequired)
        )
        .map((item) => {
          const linkComponent = {
            component: MuiLink,
  ...
  const renderNestedRoutes = routes.map(({ collapse, columns }) =>
    collapse && !columns
      ? collapse
          .filter(
            (item) =>
              !((authenticated && item.notAuthRequired) || (!authenticated && item.authRequired))
          )
          .map(({ name: parentName, collapse: nestedCollapse }) => {
            let template;
...
DefaultNavbar.propTypes = {
  ...
  routes: PropTypes.arrayOf(PropTypes.objectOf(PropTypes.oneOfType([PropTypes.object]))).isRequired,
