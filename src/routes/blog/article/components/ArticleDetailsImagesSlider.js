...
function ArticleDetailsImagesSlider({ slides }) {
  ...
  //   },
  // ];
  const steps = slides.map((item, index) => {
    const currentItem = { number: "", label: "" };
    currentItem.number = `0${index.toString()}`;
    currentItem.label = item.title;
    return currentItem;
  });
